# DayFlow System Architecture Specification

This document details the layout, architectural boundaries, and boundaries enforcement for the DayFlow project.
...
* The API structure is declared in a single source of truth: `server/src/openapi.ts`.
* Any API schema change compiles to `docs/openapi.json` and gets auto-generated into frontend TypeScript typings in `client/src/types/api.ts`.
...
* All configuration variables must be loaded and validated inside central schemas:
  - Backend: `server/src/modules/core/config/index.ts` (validated using Zod at startup).
...
### Backend
1. **Prisma Schema**: Add models inside `server/prisma/schema.prisma` and run `pnpm prisma:migrate`.
2. **API Contract**: Add endpoints to the openApi document in `server/src/openapi.ts`. Generate types with `pnpm openapi:generate`.
3. **Domain Module**: Create a new module folder in `server/src/modules/domain/<feature>/`:
...
4. **App Integration**: Register routes in `server/src/app.ts` under `/api`.

### Frontend
1. **Regenerate Types**: Compile the updated contract types using `pnpm run gen:api` in the `client` folder.
2. **API Client**: Consume endpoints using the thin API client `client/src/api/client.ts`.
3. **Views & Components**: Add route views or custom components in `client/src/components/` and register pages in `client/src/App.tsx`.
