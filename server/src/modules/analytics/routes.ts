    ...
    try {
      ...
      if (potentialWorking > 0) {
        ...
        attendancePercentage = Math.round(
          (presentToday / potentialWorking) * 100,
        );
      }
      ...
      for (const group of leaveGroups) {
        ...
        if (group.leaveType === 'UNPAID')
          leaveDistribution.unpaid = group._count;
      }
      ...
      for (const s of structures) {
        ...
        existing.net += s.basicSalary + s.allowances - s.deductions;
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!parsed.success) {
        res.status(400).json({
          error: 'Invalid query parameters. Must provide a type parameter.',
        });
        return;
      }
        ...
        const reportData = users.map((user) => {
          ...
          const totalDays = approved.reduce((sum, l) => {
            const diffTime = Math.abs(
              l.endDate.getTime() - l.startDate.getTime(),
            );
      ...
      if (type === 'attendance') {
        ...
        const reportData = users.map((user) => {
          ...
          const mockAbsent =
            leaveDaysClamped > 10 ? 0 : Math.random() > 0.85 ? 1 : 0;
          const presentDays = Math.max(
            0,
            totalWorkingDays - leaveDaysClamped - mockAbsent,
          );
